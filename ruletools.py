...
def ParsePattern(chain_str: str,
	...
	for line in lines:
		match = search(r'i\.?([xyza0-9\[\].]*)\.op\s+(?:is|==)\s+(?:idaapi\.)?(cot_\w+)', line)
		if match:
			path, op_name = match.groups()
			...
			if arg_m:
				...
				args_map[parent_path][int(idx)] = common.Slice(op_val) if isinstance(op_val, int) else op_val
			else:
				nodes[path.rstrip('.')] = op_val
